...
const DEMO_DEFS: DemoDef[] = [
  ...
  },
  {
    ...
    zoneRules: [
      {
        name: "Omakase first seating",
        days: [1, 2, 3, 4, 5, 6],
        start: "18:00",
        end: "18:00",
        step: 0,
        zones: ["Omakase counter"],
      },
      {
        name: "Omakase second seating",
        days: [1, 2, 3, 4, 5, 6],
        start: "20:00",
        end: "20:00",
        step: 0,
  ...
  },
  {
    ...
    customSlot: {
      dateOffset: 3,
      time: "15:30",
      note: "Mezze tasting walk-in seating",
    },
  },
  {
    name: "La Brasa",
    ...
    zoneRules: [
      {
        name: "Late bar & terrace",
        days: [5, 6, 0],
        start: "22:00",
        end: "23:30",
        step: 30,
        zones: ["Grill bar", "Smoking terrace"],
      },
    ],
  },
  {
...
const DEFAULT_DEMO_DAYS_AHEAD = 14;
const MAX_DEMO_DAYS_AHEAD = 14;

function boundedDaysAhead(daysAhead?: number): number {
  if (daysAhead === undefined) return DEFAULT_DEMO_DAYS_AHEAD;
  if (!Number.isFinite(daysAhead) || !Number.isInteger(daysAhead)) {
    throw new Error("Days ahead must be a whole number.");
  }
  return Math.min(MAX_DEMO_DAYS_AHEAD, Math.max(1, daysAhead));
}
...
DEMO_DEFS.forEach((d) => {
  if (!DEMO_RESTAURANT_NAMES.includes(d.name)) {
    ...
    throw new Error(
      `Demo def "${d.name}" is missing from DEMO_RESTAURANT_NAMES`,
    );
  }
});
...
export async function applyDemoRules(
  ...
  for (const demo of defs) {
    ...
    if (!opts?.force && (oldRules.length > 0 || oldCustoms.length > 0))
      continue;
    ...
    for (const zr of demo.zoneRules ?? []) {
      const sectionIds = sections
        .filter((s) => zr.zones.includes(s.name))
        .map((s) => s._id);
    ...
    await rebuildRestaurantSlots(
      ctx,
      restaurant._id,
      boundedDaysAhead(opts?.daysAhead),
    );
 ...
 *
 * The cron path is internal and can retrofit every unconfigured demo venue.
 * The public path always targets one restaurant and always verifies ownership.
 */
export const ensureDemoRulesForCron = internalMutation({
  args: {},
  handler: async (ctx) => {
    const applied = await applyDemoRules(ctx, Date.now(), {
      daysAhead: DEFAULT_DEMO_DAYS_AHEAD,
    });
    return { applied: applied.length, restaurants: applied };
  },
});

async function requireDemoRestaurantOwner(
  ctx: MutationCtx,
  restaurantName: string,
): Promise<void> {
  if (!DEMO_DEFS.some((definition) => definition.name === restaurantName)) {
    throw new Error("Unknown demo restaurant.");
  }
  const userId = await getAuthUserId(ctx);
  if (userId === null) throw new Error("You must be signed in.");
  const [caller, restaurants] = await Promise.all([
    ctx.db.get(userId),
    ctx.db.query("restaurants").collect(),
  ]);
  const restaurant = restaurants.find(
    (candidate) => candidate.name === restaurantName,
  );
  if (!restaurant) throw new Error("Restaurant not found.");
  if (caller?.role !== "admin" && restaurant.ownerId !== userId) {
    throw new Error(
      "You can only load example windows for a restaurant you own.",
    );
  }
}
...
export const ensureDemoRules = mutation({
  ...
  handler: async (ctx, args) => {
    await requireDemoRestaurantOwner(ctx, args.restaurant);
    const applied = await applyDemoRules(ctx, Date.now(), {
      restaurantName: args.restaurant,
      daysAhead: boundedDaysAhead(args.daysAhead),
      force: args.force,
    });
    return { applied: applied.length, restaurants: applied };
  },
