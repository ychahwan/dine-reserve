...
const CITIES = ["Beirut", "Dubai", "Riyadh", "Cairo", "Amman", "Kuwait City", "Doha", "Abu Dhabi", "Manama", "Muscat"];
const CUISINES = ["Lebanese", "Italian", "Japanese", "Indian", "Mexican", "Thai", "French", "Turkish", "American", "Korean"];
const NAMES_FIRST = ["Ali", "Sara", "Omar", "Layla", "Youssef", "Nadia", "Karim", "Rania", "Hassan", "Mona", "Tarek", "Jana", "Rami", "Lina", "Fadi", "Maya", "Walid", "Dina", "Samir", "Hiba"];
const NAMES_LAST = ["Khoury", "Nasser", "Haddad", "Mansour", "Saleh", "Khalil", "Amine", "Farhat", "Boustani", "Chahwan", "Hamadeh", "Itani", "Karam", "Mouawad", "Rizk"];
const SECTIONS = [
...
async function batchInsert(ctx: MutationCtx, table: string, docs: Record<string, unknown>[], batchSize = 500) {
 ...
 */
export const stressSeed = mutation({
  args: {},
  handler: async (ctx) => {
    const results = { users: 0, restaurants: 0, bookings: 0, orders: 0, reviews: 0 };
    ...
    for (let i = 0; i < 99000; i++) {
      ...
      results.users++;
      if (results.users % 5000 === 0) console.log(`Users: ${results.users}/100000`);
    }
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const id = await ctx.db.insert("restaurants", {
        ...
        city,
        neighborhood: rand(["Downtown", "Marina", "Old Town", "Hamra", "Ashrafieh"]),
        phone: `+961${String(10000000 + i).slice(0, 8)}`,
        priceRange: rand(["$", "$$", "$$$", "$$$$"]),
        description: `A great ${cuisine.toLowerCase()} restaurant in ${city}.`,
        features: { inside: true, outside: Math.random() > 0.3, bar: Math.random() > 0.5, smoking: false },
      ...
      if (results.restaurants % 100 === 0) console.log(`Restaurants: ${results.restaurants}/1000`);
    }
    ...
    for (let r = 0; r < 1000; r++) {
      ...
      for (let b = 0; b < bookingCount; b++) {
        ...
        const date = daysOffset < 0 ? pastDate(-daysOffset) : futureDate(daysOffset);
        const statuses: Array<"confirmed" | "completed" | "no_show" | "cancelled"> = ["confirmed", "completed", "completed", "completed", "no_show", "cancelled"];
    ...
    const completed = completedBookings.filter((b) => b.status === "completed").slice(0, 5000);
    for (const booking of completed) {
      const itemCount = randInt(1, 4);
      const items = Array.from({ length: itemCount }, () => ({
        name: rand(["Hummus", "Falafel", "Shawarma", "Kebab", "Fattoush", "Tabbouleh", "Grilled Halloumi", "Kibbeh"]),
