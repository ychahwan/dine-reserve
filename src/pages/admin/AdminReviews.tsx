...
type ReviewRow = NonNullable<
  ReturnType<typeof useQuery<typeof api.adminView.listReviews>>
>[number];
...
export default function AdminReviews() {
  ...
  const queriedReviews = useQuery(api.adminView.listReviews);
  const reviews = useMemo(() => capAdminRows(queriedReviews), [queriedReviews]);
  ...
  const restaurantOptions = useMemo(() => {
    ...
    for (const review of reviews ?? [])
      options.set(review.restaurantId, review.restaurantName);
    return [...options]
      .map(([id, name]) => ({ id, name }))
  ...
  const dinerOptions = useMemo(() => {
    const options = new Map<string, string>();
    for (const review of reviews ?? [])
      options.set(review.userId, review.authorName);
    return [...options]
      .map(([id, name]) => ({ id, name }))
      .sort((a, b) => a.name.localeCompare(b.name));
  }, [reviews]);
  ...
  const filtered = useMemo(() => {
    const byFilters = filterAdminReviews(reviews ?? [], {
      restaurantId,
      userId,
      rating,
    });
    if (!deferredSearchText.trim()) return byFilters;
  ...
  }, [reviews, restaurantId, userId, rating, deferredSearchText]);
  ...
  const selectedReviewIds = useMemo(
    () =>
      [...selected].filter((id) =>
        filtered.some((review) => review._id === id),
      ),
    [filtered, selected],
  );
  ...
  const handleBulkDelete = async () => {
    ...
    const results = await runBoundedParallel(
      ...
      async (reviewId) => ({
        reviewId,
        result: await removeReview({ reviewId: reviewId as never }),
      }),
    );
    for (const result of results) {
      if (result.status === "fulfilled" && result.value.result.deleted) {
        deleted.push(result.value.reviewId);
      } else {
        failed += 1;
      }
    }
    ...
    if (deleted.length > 0) {
      toast.success(
        `${deleted.length} review${deleted.length === 1 ? "" : "s"} deleted.`,
      );
    }
    if (failed > 0) {
      toast.error(
        `${failed} review${failed === 1 ? "" : "s"} could not be deleted.`,
      );
    }
  };
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {t("admin.reviewsListDesc")}
          </p>
        </div>
        <EmptyNote>
          No reviews yet — diners can rate a visit from My Bookings after it
          completes.
        </EmptyNote>
      </div>
    );
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-6">
            ...
            <div className="min-w-0">
              <p className="truncate text-[10px] text-muted-foreground sm:text-xs">
                {filtered.length < reviews.length
                  ? t("admin.avgRatingFiltered")
                  : t("admin.avgRating")}
              </p>
              <p className="text-lg font-bold tracking-tight sm:text-xl">
                {stats ? `${stats.average.toFixed(1)} / 5` : "—"}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <Card className="gap-4 py-4 shadow-none">
        <CardHeader className="px-4">
          ...
          <CardDescription>
            Combine restaurant, diner, and rating filters to narrow the
            moderation queue.
          </CardDescription>
        </CardHeader>
            ...
            <label className="flex cursor-pointer items-center gap-2 text-sm">
              ...
              <span>
                ...
                <span className="ml-1 text-muted-foreground">
                  ({selectedFilteredCount} selected here)
                </span>
              </span>
            </label>
            <Button
          ...
          <TableBody>
            {pageItems.length === 0 ? (
              ...
              pageItems.map((review) => (
                <TableRow
                  key={review._id}
                  data-state={selected.has(review._id) ? "selected" : undefined}
                >
                  <TableCell>
                    <Checkbox
            ...
            >
              ...
              <CardContent className="flex flex-col gap-3 px-4">
                <p className="line-clamp-3 text-sm leading-6 text-muted-foreground">
                  {review.text ?? (
                    <span className="italic opacity-60">No comment</span>
                  )}
                </p>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {selectedReviewIds.length} selected review
              {selectedReviewIds.length === 1 ? "" : "s"} will be permanently
              removed. Loyalty points linked to each review will be reversed,
              and every admin deletion will be audited.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
