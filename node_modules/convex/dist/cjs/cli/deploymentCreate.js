...
const deploymentCreate = new import_extra_typings.Command("create").summary("Create a new deployment for a project").description(
  ...
  (0, import_announceDeploymentTarget.announceDeploymentTarget)(null, {
...
async function createLocalDeployment(ctx, currentDeployment, select, baseDeployment) {
  ...
  const {
    ...
    slug: projectSlug,
    id: cloudProjectId
  } = baseDeployment ? await (0, import_deploymentSelection.getProjectDetails)(ctx, {
    kind: "teamAndProjectSlugs",
  ...
  switch (selector.kind) {
    ...
    case "inTeamProject": {
      ...
      if (inner.kind === "local") {
        return {
          kind: "invalid",
          message: `"local" is reserved as an alias for your local deployment. To create one, run ${import_chalk.chalkStderr.bold(`npx convex deployment create ${selector.teamSlug}:${selector.projectSlug}:local`)}`
        };
      }
      return {
        kind: "valid",
...
function parseLocalCreateTarget(refParam) {
  const parsed = (0, import_deploymentSelector.parseDeploymentSelector)(refParam);
