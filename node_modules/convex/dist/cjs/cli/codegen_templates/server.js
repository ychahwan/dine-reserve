...
function generateEnvInterface(envVars) {
  const lines = [];
  for (const [name, decl] of envVars) {
    const parsed = (0, import_validator_helpers.parseValidator)(decl.value);
    let tsType = parsed ? (0, import_validator_helpers.validatorToType)(parsed, false) : "unknown";
    if (decl.optional) {
      tsType += " | undefined";
    }
