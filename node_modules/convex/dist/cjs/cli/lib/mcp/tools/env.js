...
const EnvSetTool = {
  ...
  handler: async (ctx, args) => {
    ...
    const backend = (0, import_env.deploymentEnvBackend)(ctx, deploymentInfo);
    await (0, import_env.envSet)(ctx, backend, args.name, args.value);
    return { success: true };
  }
};
...
const EnvRemoveTool = {
  ...
  handler: async (ctx, args) => {
    ...
    const backend = (0, import_env.deploymentEnvBackend)(ctx, deploymentInfo);
    await (0, import_env.envRemove)(ctx, backend, args.name);
    return { success: true };
  }
};
