...
__export(codegen_exports, {
  cleanupStaleGeneratedEntries: () => cleanupStaleGeneratedEntries,
  doCodegen: () => doCodegen,
  doFinalComponentCodegen: () => doFinalComponentCodegen,
  doInitConvexFolder: () => doInitConvexFolder,
...
const PRESERVED_GENERATED_ENTRIES = /* @__PURE__ */ new Set(["ai"]);
function cleanupStaleGeneratedEntries(ctx, codegenDir, writtenFiles, opts) {
  if (opts?.debug) {
    return;
  }
...
async function doInitConvexFolder(ctx, functionsFolder, opts) {
  ...
  if (functionsFolder) {
    ...
    folder = (0, import_utils.functionsDir)(configPath, projectConfig);
  }
...
async function prepareForCodegen(ctx, functionsDir2, opts) {
  const functionsDirExistedBeforeCodegen = ctx.fs.exists(functionsDir2);
...
async function doFinalComponentCodegen(ctx, tmpDir, rootComponent, componentDirectory, startPushResponse, componentsMap, opts) {
  ...
  const envVars = analysis?.definition.envVars && analysis.definition.envVars.length > 0 ? analysis.definition.envVars : [];
...
async function doReadmeCodegen(ctx, tmpDir, functionsDir2, skip, opts) {
...
async function writeServerFiles(ctx, tmpDir, codegenDir, useTypeScript, envVars, opts) {
  if (!useTypeScript) {
...
async function writeComponentServerFile(ctx, tmpDir, codegenDir, envVars, opts) {
  const serverTSPath = import_path.default.join(codegenDir, "server.ts");
  const serverTSContents = (0, import_component_server.componentServerTS)(false, envVars);
  await writeFormattedFile(
    ctx,
    tmpDir,
    serverTSContents,
...
async function writeServerFilesForComponent(ctx, isRoot, tmpDir, codegenDir, useTypeScript, envVars, opts) {
  if (isRoot) {
...
async function doInitialComponentServerCodegen(ctx, isRoot, tmpDir, codegenDir, useTypeScript, envVars, opts) {
  const serverFilename = useTypeScript ? "server.ts" : "server.js";
  if (ctx.fs.exists(import_path.default.join(codegenDir, serverFilename))) {
