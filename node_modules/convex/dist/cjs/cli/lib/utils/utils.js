...
__export(utils_exports, {
  ...
  logNoDefaultRegionMessage: () => logNoDefaultRegionMessage,
  parseInteger: () => parseInteger,
  parsePositiveInteger: () => parsePositiveInteger,
  permissionDeniedErrorMessage: () => permissionDeniedErrorMessage,
  poll: () => poll,
...
const PERMISSION_DENIED_MESSAGE_PREFIX = "You do not have permission to perform this operation";
function deployKeyPermissionAdvice() {
  const envVarName = process.env[CONVEX_DEPLOY_KEY_ENV_VAR_NAME] ? CONVEX_DEPLOY_KEY_ENV_VAR_NAME : process.env[CONVEX_DEPLOYMENT_TOKEN_ENV_VAR_NAME] ? CONVEX_DEPLOYMENT_TOKEN_ENV_VAR_NAME : null;
  if (envVarName === null) {
    return null;
  }
  return `This is determined by the permissions granted to ${envVarName}.`;
}
function permissionDeniedErrorMessage(errorMessage) {
...
async function logNoDefaultRegionMessage(teamSlug) {
  const { teamDashboardUrl } = await import("../dashboard.js");
  const noDefaultRegionMessage = import_chalk.chalkStderr.gray(
    `Tip: you can configure a default region for your team at ${import_chalk.chalkStderr.underline(`${teamDashboardUrl(teamSlug)}/settings`)}`
  );
