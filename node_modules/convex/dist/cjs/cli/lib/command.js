...
import_extra_typings.Command.prototype.addRunOptions = function() {
  return this.argument(
    ...
    new import_extra_typings.Option(
      "--inline-query <query>",
      import_runTestFunction.INLINE_QUERY_DESCRIPTION
    ).conflicts("--watch")
...
import_extra_typings.Command.prototype.addLogsOptions = function() {
  return this.option(
    ...
    false
  ).option("--jsonl", "Output raw log events as JSONL", false).addOption(
    new import_extra_typings.Option(
      "--tail [n]",
    ...
    ).argParser(import_utils.parseInteger).hideHelp()
  );
};
