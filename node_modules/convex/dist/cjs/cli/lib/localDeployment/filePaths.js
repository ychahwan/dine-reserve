...
__export(filePaths_exports, {
  ...
  loadGlobalDashboardConfig: () => loadGlobalDashboardConfig,
  loadProjectDashboardConfig: () => loadProjectDashboardConfig,
  ...
  saveGlobalDashboardConfig: () => saveGlobalDashboardConfig,
  saveProjectDashboardConfig: () => saveProjectDashboardConfig,
...
function loadGlobalDashboardConfig(ctx) {
...
function saveGlobalDashboardConfig(ctx, config) {
  const configFile = import_path.default.join(dashboardDir(), "config.json");
...
function loadProjectDashboardConfig(ctx, deploymentName) {
  ...
  try {
    ...
    (0, import_log.logVerbose)(`Failed to parse project dashboard config: ${e}`);
    return null;
  }
}
function saveProjectDashboardConfig(ctx, deploymentName, config) {
  const configPath = dashboardConfigPath(ctx, deploymentName);
  const directoryPath = import_path.default.dirname(configPath);
