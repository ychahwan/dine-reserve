...
import type { paths as CliManagementPaths, RegionName, TeamResponse } from "../../generatedApi.js";
...
export declare const CONVEX_DEPLOYMENT_TOKEN_ENV_VAR_NAME = "CONVEX_DEPLOYMENT_TOKEN";
...
export declare function selectDevDeploymentType(ctx: Context, { chosenConfiguration, newOrExisting, teamSlug, projectSlug, userHasChosenSomethingInteractively, devDeploymentFromFlag, }: {
...
export declare function logNoDefaultRegionMessage(teamSlug: string): Promise<void>;
export declare function selectRegionOrUseDefault(ctx: Context, selectedTeam: TeamResponse, deploymentType: CloudDeploymentType): Promise<"aws-us-east-1" | "aws-eu-west-1" | null>;
export declare function selectRegion(ctx: Context, teamId: number, deploymentType: CloudDeploymentType): Promise<RegionName>;
 ...
 */
export declare function bigBrainFetch(ctx: Context): typeof fetch;
...
export declare function typedDeploymentClient(ctx: Context, args: {
    deploymentUrl: string;
    adminKey: string;
}, options?: {
    throw?: boolean;
}): import("openapi-fetch").Client<PlatformDeploymentPaths, `${string}/${string}`>;
