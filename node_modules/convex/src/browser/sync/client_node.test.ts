...
// https://github.com/get-convex/convex-js/issues/148
// When using expectAuth, the socket starts paused. Once the WebSocket connects
// (while still paused), the paused state becomes "uninitialized". When auth
// completes and resume() is called, hasEverConnected should become true.
test("hasEverConnected is true after expectAuth resume", async () => {
  await withInMemoryWebSocket(async ({ address, receive, send }) => {
    // expectAuth causes the socket to start in paused state.
    const client = new BaseConvexClient(address, () => null, {
      webSocketConstructor: nodeWebSocket,
      unsavedChangesWarning: false,
      expectAuth: true,
    ...
    // Before auth, socket is paused so hasEverConnected should be false.
    expect(client.connectionState().hasEverConnected).toBe(false);

    // Set auth with a delayed token fetch. The delay ensures the WebSocket
    // connects while the token is being fetched, causing the socket to enter
    // the ready+uninitialized state (the buggy path).
    const iat = Math.floor(Date.now() / 1000);
    const exp = iat + 3600;
    const token = encodeJwt({ sub: "user_123", exp, iat }, "test-secret");
    client.setAuth(
    ...
    // After resume, the deferred onOpen fires and sends Connect + auth messages.
    expect((await receive()).type).toEqual("Connect");
    expect((await receive()).type).toEqual("Authenticate");
    expect((await receive()).type).toEqual("ModifyQuerySet");

    // Respond to the handshake noting that the identity was accepted.
    send({
      type: "Transition",
      startVersion: { querySet: 0, identity: 0, ts: Long.fromNumber(0) },
      endVersion: { querySet: 0, identity: 1, ts: Long.fromNumber(100) },
    ...
    // Let the client process the transition.
    await new Promise((r) => setTimeout(r, 100));

    // After auth completes and socket is resumed, hasEverConnected should be true.
