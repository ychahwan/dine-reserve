...
describe("deployment selection flows", () => {
  ...
  describe("regular command (npx convex env)", () => {
    ...
    describe("--deployment flag", () => {
      ...
      it("--deployment local with matching cloudProjectId resolves to local credentials", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
      ...
      it("--deployment local with mismatching cloudProjectId crashes with helpful message", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
        ...
        // Did not call bigBrainPause or bigBrainStart (read-only path)
        expect(bigBrainAPI).not.toHaveBeenCalledWith(
          expect.objectContaining({ path: "local_deployment/pause" }),
        );
        expect(bigBrainAPI).not.toHaveBeenCalledWith(
          expect.objectContaining({ path: "local_deployment/start" }),
        );
      });

      it("--deployment local with missing cloudProjectId skips check and resolves", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
      ...
      it("--deployment project:local with matching cloudProjectId resolves to local credentials", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
        ...
        await env.parseAsync(
          ["set", "ABC", "DEF", "--deployment", "my-project:local"],
          { from: "user" },
        );
      ...
      it("--deployment project:local with missing cloudProjectId resolves", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
      ...
      it("--deployment project:local with mismatching cloudProjectId crashes", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-old_team-old_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
      ...
      it("--deployment team:project:local with matching cloudProjectId resolves", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
        ...
        await env.parseAsync(
          ["set", "ABC", "DEF", "--deployment", "my-team:my-project:local"],
          { from: "user" },
        );
      ...
      it("--deployment team:project:local with missing cloudProjectId resolves", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-my_team-my_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
      ...
      it("--deployment team:project:local with mismatching cloudProjectId crashes", async () => {
        vi.mocked(loadProjectLocalConfig).mockReturnValue({
          deploymentName: "local-old_team-old_project-abc",
          config: {
            ports: { cloud: 3210, site: 3211 },
        ...
        await expect(
          env.parseAsync(
            [
              ...
              "DEF",
              "--deployment",
              "other-team:other-project:local",
            ],
            { from: "user" },
          ),
        ).rejects.toThrow();
  ...
  describe("deploy command (npx convex deploy)", () => {
    ...
    it("reuses preview deployment with preview deploy key and --preview-name", async () => {
      process.env.CONVEX_DEPLOY_KEY = "preview:my-team:my-project|secretkey";
      ...
      await deploy.parseAsync(["--preview-name", "my-preview"], {
        from: "user",
      });
  ...
  describe("dev command (npx convex dev)", () => {
    test("dev --local is deprecated and points users to `deployment select local`", async () => {
      await expect(
        dev.parseAsync(["--local"], { from: "user" }),
      ).rejects.toThrow();

      expect(process.stderr.write).toHaveBeenCalledWith(
        expect.stringContaining("`--local` is deprecated"),
      );
      expect(process.stderr.write).toHaveBeenCalledWith(
        expect.stringContaining("npx convex deployment select local"),
      );
    ...
    test("dev --cloud is deprecated and points users to `deployment select dev`", async () => {
      await expect(
        dev.parseAsync(["--cloud"], { from: "user" }),
      ).rejects.toThrow();

      expect(process.stderr.write).toHaveBeenCalledWith(
        expect.stringContaining("`--cloud` is deprecated"),
      );
      expect(process.stderr.write).toHaveBeenCalledWith(
        expect.stringContaining("npx convex deployment select dev"),
      );
      expect(devAgainstDeployment).not.toHaveBeenCalled();
    });
