 ...
 * - All runner calls are awaited (the runner is async — see scripts/lib/runner.mjs).
...
(async function cleanupHarnessRestaurants() {
  ...
  for (const { id: rId, owner } of pairs) {
    if (!owner) continue;
    await runfn("restaurants:remove", JSON.stringify({ id: rId }), "--identity", id(owner));
  }
...
await runfn("restaurants:create", JSON.stringify({
...
await check("C-1b detail renders for identity-subject owner", RNAME, "restaurants:get", JSON.stringify({ id: RID }));
...
await runfn("restaurants:addSection", JSON.stringify({
  restaurantId: RID, name: "UI Counter", kind: "inside", smoking: false, capacity: 2,
}), "--identity", id(OWNER));
await check("C-2b 2-seat section shows in detail", "UI Counter", "restaurants:get", JSON.stringify({ id: RID }));
await runfn("restaurants:deleteSection", JSON.stringify({ id: DEFSEC }), "--identity", id(OWNER));
await runfn("restaurants:saveHours", JSON.stringify({
  restaurantId: RID,
  hours: Array.from({ length: 7 }, (_, dayOfWeek) => ({ dayOfWeek, open: "17:00", close: "22:00", enabled: true })),
}), "--identity", id(OWNER));
await check("C-3b slots materialize", "created: 10", "availability:ensureForDate",
...
await check("C-4b direct booking fills 19:00", "status: 'confirmed'", "bookings:createBooking", JSON.stringify({
...
// ---------------------------------------------------------------- G-5 invite page
await check("G-5 byCode returns the booking", "status: 'confirmed'", "bookings:byCode", JSON.stringify({ code: CODE }));
await check("G-5b byCode includes restaurant", RNAME, "bookings:byCode", JSON.stringify({ code: CODE }));

// ---------------------------------------------------------------- D-7 diner notify
// sendForBooking returns the booking (the alert is verified via myAlerts below)
await check("D-7 diner sends running_late alert", "status: 'confirmed'", "notifications:sendForBooking", JSON.stringify({
  bookingId: BOOK2, type: "running_late", message: "Traffic, 15 min late",
}), "--identity", id(DINER2));
await check("D-7b diner sees own alert (myAlerts)", "running_late", "notifications:myAlerts", "{}", "--identity", id(DINER2));
...
await check("D-6 unread badge > 0", "ok: true", "--inline-query",
...
await check("D-6b per-booking filter", "running_late", "notifications:forRestaurant",
  JSON.stringify({ restaurantId: RID, bookingId: BOOK2 }), "--identity", id(OWNER));
await check("D-6c diner name falls back to Guest", "dinerName: 'Guest'", "notifications:forRestaurant",
  JSON.stringify({ restaurantId: RID }), "--identity", id(OWNER));
await runfn("notifications:markAllRead", JSON.stringify({ restaurantId: RID }), "--identity", id(OWNER));
await check("D-6d mark all read clears badge", "unread: 0", "--inline-query",
...
const MENU = await iq(
  ...
  await runfn("restaurants:createMenu", JSON.stringify({ restaurantId: RID, name: "Dinner" }), "--identity", id(OWNER));
...
await runfn("restaurants:createMenuItem", JSON.stringify({
...
await check("E-8 add item w/ image + attributes", "Smoked Cacio", "--inline-query",
...
await check("E-8b hide item (available=false)", "available: false", "restaurants:updateMenuItem", JSON.stringify({
  id: ITEM, available: false, priceCents: 1590,
}), "--identity", id(OWNER));
await check("E-8c invalid spice rejected", "Invalid spice level", "restaurants:updateMenuItem", JSON.stringify({
  id: ITEM, spiceLevel: "nuclear",
}), "--identity", id(OWNER));
await check("E-8d item photo persisted", "images.example.com", "--inline-query",
...
await check("E-8e allergen + spice data persisted", "Dairy", "--inline-query",
...
await check("B-10 menu tags on detail", "Gluten-free", "--inline-query",
...
await check("F-6 owner completes the visit", "status: 'completed'", "bookings:updateStatus", JSON.stringify({
  bookingId: BOOK2, status: "completed",
}), "--identity", id(OWNER));
await check("F-6b diner sees it as reviewable", "UI Counter", "reviews:myReviewable", "{}", "--identity", id(DINER2));
await check("F-6c verified review created", "rating: 5", "reviews:create", JSON.stringify({
  bookingId: BOOK2, rating: 5, text: "UI-flow review.",
}), "--identity", id(DINER2));
await check("F-6d duplicate rejected", "already reviewed", "reviews:create", JSON.stringify({
  bookingId: BOOK2, rating: 4,
}), "--identity", id(DINER2));
await check("F-6e rating aggregate on detail", "avg: 5", "reviews:listForRestaurant", JSON.stringify({ restaurantId: RID }));
await check("F-6f review no longer reviewable", "[]", "reviews:myReviewable", "{}", "--identity", id(DINER2));
...
const favState = await iq(
  `const u = await ctx.db.get("${AVA}"); return { fav: (u?.favorites ?? []).includes("${RID}") };`,
);
if (favState.includes("true")) {
  await runfn("users:toggleFavorite", JSON.stringify({ restaurantId: RID }), "--identity", id(AVA));
}
await check("G-6 favorite on", "favorited: true", "users:toggleFavorite", JSON.stringify({ restaurantId: RID }), "--identity", id(AVA));
await check("G-6b favorites list shows it", RNAME, "users:myFavorites", "{}", "--identity", id(AVA));
await check("G-6c favorite off", "favorited: false", "users:toggleFavorite", JSON.stringify({ restaurantId: RID }), "--identity", id(AVA));

// ---------------------------------------------------------------- B-9 explore data + security
await check("B-9 search returns UI-flow restaurant", RNAME, "restaurants:search", "{}");
await check("UI-1 non-owner cannot see bookings", "[]", "bookings:byRestaurant",
  JSON.stringify({ restaurantId: RID }), "--identity", id(DINER));
await check("UI-2 signed-out booking rejected", "Please sign in", "bookings:createBooking", JSON.stringify({
