...
|----------|----------|-------------|
| `JWKS` | Yes | Auth public keys (auto-generated by Convex Auth) |
| `JWT_PRIVATE_KEY` | Yes | Auth signing key (auto-generated by Convex Auth) |
| `SITE_URL` | Yes | Your app's public URL for auth redirects |
| `PLATFORM_ADMIN_PHONE` | No | Phone number that can claim platform admin role (default: `+96176683661`). Format: E.164, e.g. `+1234567890` |
| `TWILIO_ACCOUNT_SID` | No | Twilio SMS (graceful no-op without) |
| `TWILIO_AUTH_TOKEN` | No | Twilio SMS |
| `TWILIO_FROM_NUMBER` | No | Twilio SMS sender number |

### E2E test environment (optional, for Playwright tests)

| Variable | Description |
|----------|-------------|
| `E2E_ADMIN_PHONE` | Admin phone for E2E tests (default: `+96176683661`) |
| `E2E_ADMIN_PASSWORD` | Admin password for E2E tests (default: `BeityAdmin2026!`) |
...
The platform admin is the only account that can register restaurants, tag accounts
as restaurants, and moderate the platform. Everything an admin does is written to
the `adminAuditLog` table and visible in the admin console under **Audit log**.

### First-time setup

1. **Set the admin phone** (your own phone, in E.164 format):
   ```bash
   npx convex env set PLATFORM_ADMIN_PHONE "+1234567890"
   ```
   This is the phone number that will be allowed to claim the admin role. Keep
   it secret — it's effectively the root credential.

2. **Deploy the backend** (push functions to Convex):
   ```bash
   npx convex dev          # dev: auto-redeploy on save
   # or
   npx convex deploy       # production deploy
   ```

3. **Open the app** and sign in with the admin phone via OTP (phone-otp auth):
   - Go to `/auth` → enter the admin phone → receive OTP → sign in.

4. **Claim the admin role** — call `admin.claimPlatformAdmin` from the Convex dashboard:
   - Open [Convex Dashboard](https://dashboard.convex.dev) → your deployment
   - Go to **Functions** → search `admin.claimPlatformAdmin` → **Run as** the admin user
   - Or call it from the browser console after signing in:
     ```javascript
     // In the browser console (while signed in as the admin phone)
     ConvexClient.mutation("admin.claimPlatformAdmin", {})
     ```

5. **Set a password** for the admin account (so you can log in with password, not just OTP):
   - Open the admin console at `/admin` → Users → find your admin account
   - Or call `admin.setUserPassword` from the Convex dashboard:
     ```bash
     npx convex run admin.setUserPassword -- --userId <ADMIN_USER_ID> --newPassword "YourSecurePassword"
     ```

6. **Log in with password** — now you can use `/auth` → phone + password instead of OTP.

> **Security:** The admin password is never stored in source control. Set it
> during initial setup and rotate it periodically. The admin phone is the only
> value that can bootstrap admin access.
...
The app does **not** auto-seed demo data. You control when to create demo data
via admin-only mutations. This keeps production deployments clean.

### Seed a fresh demo dataset

After signing in as admin, call `resetData` to wipe everything and recreate
the full demo dataset (100 customers, 5 restaurants with menus, bookings,
waitlist, and reviews):

```bash
# Via Convex dashboard: Functions → seed.resetData → Run as admin
# Or via CLI:
...
|----------|-------|---------|
| Customers | 100 | Named demo accounts (`@kamix.demo`) |
| Restaurants | 5 | Italian, Japanese, Lebanese, Mexican, International |
| Sections | 11 | Dining rooms, terraces, bars, counters |
| Menus | 5 | Full menus with items, allergens, ingredients |
| Bookings | 3 | Sample bookings for today/tomorrow |
| Waitlist | 1 | Sold-out Sakura omakase slot |
| Reviews | 3 | Verified reviews from demo customers |
| Gift catalog | 12 | Socialize gifts per venue |
| AI knowledge | 5 | Default semantic rules for the AI concierge |

### Generate realistic activity history

To backfill ~5 weeks of bookings, orders, reviews, and waitlist entries for
analytics and the AI concierge:

```bash
# Via Convex dashboard: Functions → seed.generateDemoActivity → Run as admin
```

Idempotent — skips restaurants that already have >15 bookings.

### Available seed mutations

| Mutation | What it does |
|----------|-------------|
| `seed.resetData` | Wipe all app data + reseed from scratch |
| `seed.seed` | Seed base data only (no wipe, idempotent — skips if data exists) |
| `seed.generateDemoActivity` | Generate 5 weeks of realistic booking/order/review history |
| `seed.ensureDemoData` | Check if demo data exists (returns `{ seeded: boolean }`) |
| `seed.wipeAllData` | Delete all app data (keeps auth accounts) |
...
**Warning:** Only run on a test deployment.
